 ...
 */
public class PursueBackAttackPolicyListener implements PolicyListener {
	...
	public void execute(RoleFight roleFight, ArmyPhalanx armyPhalanx, ArmyPhalanx[] armyPhalanxList, int errorTime) {
		...
		{
			ArmyPhalanx armyAimPhalanx = calculate.getAttackAim(armyPhalanx, armyPhalanxList, gameMap);
		...
		if (armyPhalanx.getAimPhalanxId() == null) {
			if (controlType != 0) {
				...
				armyPhalanx.setPolicy(Policy.PURSUE_BACK_AUTO_ATTACK);
		...
		if (armyPhalanx.getCurrPursueTime() >= armyPhalanx.getMaxPursueTime()) {
			...
			if(path == null){
				...
				Step endStep = path.getEndStep();
				if(!gameMap.worldToGrid(endStep.getX(), endStep.getY()).equals(new Point(armyPhalanx.getPursuePreX(), armyPhalanx.getPursuePreY())))
				{
					armyPhalanx.setPath(null);
					calculate.setArmyPhlanxStagePoint(armyPhalanx, gameMap);
			...
			// 进行移动
			path = calculate.getAimRankList(gameMap, armyPhalanx, armyPhalanx.getCurrX(), armyPhalanx.getCurrY(), armyPhalanx.getPursuePreX(), armyPhalanx.getPursuePreY(), false, roleFight.getFightMapType(), roleFight.getFightId());
