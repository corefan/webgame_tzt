package com.snail.webgame.engine.component.scene.protocal.reset;
...
public class ResetPositionProcessor extends ProtocolProcessor {
	...
	public void execute(Request request, Response response) {
		...
		ResetPositionReq req = (ResetPositionReq) message.getBody();
		GameMessageHead head = (GameMessageHead) message.getHeader();

		int roleId = head.getUserID0();
		
		sceneMgtService.resetPosition(roleId);
	
		if (logger.isInfoEnabled()) {
			logger.info(Resource.getMessage("game", "GAME_SCENE_INFO_11")
							+ ",roleId=" + roleId);
		}
